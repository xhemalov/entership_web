...
// SVG Icons with cream color
const LightningIcon = () => (
  <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="#FFF8E7" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <Accordion type="single" collapsible className="space-y-4">
          {faqs.map((faq, index) => (
            <AccordionItem 
              key={index} 
              value={`item-${index}`}
              className="bg-purple-light/20 border-purple-light/30 rounded-lg px-6"
            >
              <AccordionTrigger className="text-cream font-inter font-semibold text-left hover:text-yellow-accent">
                <div className="flex items-center gap-3">
                  {faq.icon}
                  <span>{faq.question}</span>
                </div>
              </AccordionTrigger>
              <AccordionContent className="text-purple-200 font-inter leading-relaxed">
                {faq.answer}
              </AccordionContent>
            </AccordionItem>
          ))}
